...
export default class TabSection extends Component {
	...
	renderTab = () => {
		...
		if (this.props.subtab !== this.state.current_url) {
			axios({
				url: `${termsconditionsappLocalizer.apiUrl}/woo_checkout_terms_conditions/v1/fetch_admin_tabs`,
			}).then((response) => {
				this.setState({
									...
									{Object.entries(model).length > 0 ? Object.entries(model).map((m, index) => {
										return m[1].link ? (	
											...
											<Link
												...
												to={
													`?page=terms_conditions_popup#&tab=${query_name}&subtab=${m[0]}`
												}
											>
												{m[1].icon ? (
													<i
							...
							<div className="woo-tab-content">
								{
									...
									<DynamicForm
										...
										method="post"
										modulename={m[0]}
										url={`woo_checkout_terms_conditions/v1/${m[1].apiurl}`}
										submitbutton="false"
									/>
									: (
										''
